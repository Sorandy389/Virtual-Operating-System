...
#define TERMINAL_NUM	0x03	// the number of terminals support
#define KEYBOARD_BUF_SIZE	0x80	// the size of the keyboard buffer
#define NUM_COLS    80
#define NUM_ROWS    25
#define ATTRIB      0x7
...
/* terminal structure */
struct terminal{
	uint8_t video_memory[2 * NUM_ROWS * NUM_COLS];
	int8_t key_buf[KEYBOARD_BUF_SIZE];
	int32_t num_buf;
	int8_t buf_flag;
	int32_t screen_x;
	int32_t screen_y;
	int32_t start_line_ptr;
	int32_t process_num;
	int32_t flag;
	int32_t total_process;
} terminal_t[TERMINAL_NUM];

/* the number of current terminal */
volatile int32_t terminal_num;
volatile int32_t terminal_displayed;
